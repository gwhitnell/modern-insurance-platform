...
resource "snowflake_grant_privileges_to_account_role" "loader_prd_database_usage" {
  account_role_name = snowflake_account_role.loader_role.name
  privileges        = ["USAGE"]

  on_account_object {
    object_type = "DATABASE"
    object_name = snowflake_database.env_db["PRD"].name
  }
...
resource "snowflake_grant_privileges_to_account_role" "transform_prd_database_usage" {
  account_role_name = snowflake_account_role.transform_role.name
  privileges        = ["USAGE"]
...
resource "snowflake_grant_privileges_to_account_role" "analyst_prd_database_usage" {
  account_role_name = snowflake_account_role.analyst_role.name
  privileges        = ["USAGE"]
